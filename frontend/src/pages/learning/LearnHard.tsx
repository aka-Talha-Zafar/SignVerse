...
export default function LearnHard() {
  ...
  const handleMarkLearned = (sentenceId: string) => {
    markSentenceCompleted(sentenceId);
    setCompletedSentences((prev) => (prev.includes(sentenceId) ? prev : [...prev, sentenceId]));
  };
      ...
      <div className="min-h-screen bg-gray-950 text-white">
        <header className="sticky top-0 z-50 border-b border-gray-800 bg-gray-950/90 backdrop-blur-xl">
          <div className="max-w-7xl mx-auto px-6 h-14 flex items-center gap-4">
            <button onClick={() => setSelectedSentence(null)} className="text-gray-400 hover:text-white transition-colors flex items-center gap-2">
        ...
        <main className="max-w-3xl mx-auto px-6 py-10">
          <div className="flex flex-col items-center space-y-6">
            ...
            <div className="flex gap-3 w-full max-w-sm">
              <button
                onClick={() => {
                  if (idx > 0) setSelectedSentence(selectedCategory.sentences[idx - 1]);
                }}
                disabled={idx === 0}
              ...
              <button
                onClick={() => {
                  ...
                  if (idx < selectedCategory.sentences.length - 1) {
                    setSelectedSentence(selectedCategory.sentences[idx + 1]);
                  } else {
          ...
          {SENTENCE_CATEGORIES.map((cat) => {
            ...
            return (
              ...
              >
                ...
                <p className="text-xs text-gray-500 mt-1">{done}/{cat.sentences.length} completed</p>
              </button>
            );
          })}
        </div>
